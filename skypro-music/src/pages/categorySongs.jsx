...
import { catalogClassic, catalogElectro, catalogRock } from "../store/slices/trackSlice";
...
export const CategorySongs = () => {
  ...
  useEffect(() => {
    ( async () => {const tracks = await getCatalog_1(token)
  ...
  useEffect(() => {
    ( async () => {const tracks = await getCatalog_2(token)
      console.log(tracks)
  ...
  useEffect(() => {
    ( async () => {const tracks = await getCatalog_3(token)
      console.log(tracks)
    ...
    return (
      <>
        {params.id === "1" ? <S.centoblockTittle>Классическая музыка</S.centoblockTittle> : null}
        {params.id === "2" ? <S.centoblockTittle>Электронная музыка</S.centoblockTittle> : null}
        {params.id === "3" ? <S.centoblockTittle>Рок музыка</S.centoblockTittle> : null}
        {params.id === "1" ?
          classic.map((item) => {
